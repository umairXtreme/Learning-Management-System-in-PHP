...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($old_password) || !empty($new_password) || !empty($confirm_password)) {
        if (strlen($new_password) < 8) {
            ...
            header("Location: instructor-settings.php");
            exit;
        } elseif (!password_verify($old_password, $instructor['password'])) {
    ...
    if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] === 0) {
        ...
        if (in_array($ext, $allowedTypes)) {
            $newFileName = "instructor_" . $instructorId . "_" . time() . "." . $ext;
    ...
    if (isset($_FILES['resume']) && $_FILES['resume']['error'] === 0) {
        ...
        if (in_array($resumeExt, $allowedResumeExt)) {
            $resumeFileName = "resume_" . $instructorId . "_" . time() . "." . $resumeExt;
...
<div class="main-content">
    <div class="admin-settings-wrapper">
        <h4 class="mb-4"><i class="fas fa-user-cog me-2"></i> Instructor Settings</h4>
        <form method="POST" enctype="multipart/form-data" class="row g-4">
            <div class="col-md-4 text-center">
                <div class="profile-picture-wrapper">
                    <img id="preview" src="<?= $instructor['profile_picture'] ? '../assets/uploads/profileimages/' . $instructor['profile_picture'] : 'https://via.placeholder.com/170?text=Avatar' ?>" class="img-fluid shadow" alt="Profile Picture">
                    <input type="file" name="profile_picture" class="form-control mt-3" onchange="previewImage(event)">
                </div>
            </div>
            <div class="col-md-8">
                <div class="mb-3"><label class="form-label">Username</label><input type="text" class="form-control" value="<?= htmlspecialchars($instructor['username']) ?>" disabled></div>
                <div class="mb-3"><label class="form-label">Full Name</label><input type="text" class="form-control" name="full_name" value="<?= htmlspecialchars($instructor['full_name']) ?>" required></div>
                <div class="mb-3"><label class="form-label">Email</label><input type="email" class="form-control" name="email" value="<?= htmlspecialchars($instructor['email']) ?>" required></div>
                <div class="mb-3"><label class="form-label">Phone Number</label><input type="text" class="form-control" name="phone" value="<?= htmlspecialchars($instructor['phone']) ?>" required></div>
                <div class="mb-3">
                    <label class="form-label">Upload Resume</label>
                    <input type="file" class="form-control" name="resume">
                    <?php if ($instructor['resume']): ?>
                        <a href="<?= $resumeDir . $instructor['resume'] ?>" target="_blank" class="text-success d-block mt-1">📄 View Current Resume</a>
                    <?php endif; ?>
                </div>
                <div class="mb-3"><label class="form-label">Account Created At</label><input type="text" class="form-control" value="<?= htmlspecialchars($instructor['created_at']) ?>" disabled></div>
                <hr>
